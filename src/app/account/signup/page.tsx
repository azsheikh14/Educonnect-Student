...
const Signup: React.FC = () => {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 p-6">
      <div className="relative w-[60vw] bg-white p-8 rounded-lg shadow-lg flex">
        <div className="w-1/2 p-6">
          <h1 className="text-3xl font-semibold mb-12 text-center text-gray-800">Create Your Account</h1>
          {error && (
            <p className="transform -translate-x-1/2 text-red-500">
              {error}
            </p>
          )}
          {message && (
            <p className="absolute top-20 left-1/2 transform -translate-x-1/2 text-green-500">
              {message}
            </p>
          )}
          ...
          <div className="mt-4 text-center flex items-center">
            ...
            <button onClick={() => router.push('/account/login')} className="text-blue-600 hover:underline ml-2">
              Go to Login
            </button>
          </div>
        </div>
        <div className="w-1/2 flex items-center justify-center bg-gray-100 rounded-r-lg">
          <img src="/signup.png" alt="Signup illustration" className="w-full h-full object-cover rounded-r-lg" />
        </div>
      </div>
    </div>
  );
