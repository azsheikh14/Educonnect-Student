import React, { useRef, useState } from 'react';
import Image from 'next/image';
import Link from 'next/link';
import Notification from '@/app/interfaces/Notification'

interface DropdownNotification extends Notification {
  removing?: boolean;
}

interface NotificationComponentProps {
  notifications: DropdownNotification[];
  removeNotification: (index: number) => void;
  closeNotifications: () => void;
}

const NotificationComponent: React.FC<NotificationComponentProps> = ({ notifications, removeNotification, closeNotifications }) => {
  const dropdownRef = useRef<HTMLDivElement>(null);
  const [visibleNotifications, setVisibleNotifications] = useState<DropdownNotification[]>(notifications);
  console.log('visibleNotifications :', visibleNotifications);

  const handleRemoveNotification = (index: number) => {
    const newNotifications = [...visibleNotifications];
    newNotifications[index] = { ...newNotifications[index], removing: true };

    setVisibleNotifications(newNotifications);

    setTimeout(() => {
      newNotifications.splice(index, 1);
      setVisibleNotifications(newNotifications);
      removeNotification(index);
    }, 300);
  };

  return (
    <div ref={dropdownRef} className="absolute right-16 top-12 mt-2 w-96 bg-white border border-gray-200 shadow-lg z-50 rounded-lg">
      <div className="relative">
        <div className="absolute -top-2 right-4 w-4 h-4 bg-white border-l border-t border-gray-200 transform rotate-45"></div>
      </div>
      <div className={`max-h-80 scrollbar-thin scrollbar-track-rounded scrollbar-thumb-rounded overflow-y-auto ${visibleNotifications.length === 0 ? '' : 'h-auto'}`}>
        {visibleNotifications.length === 0 ? (
          <div className="p-4 text-gray-500">No notifications</div>
        ) : (
          visibleNotifications.slice().reverse().map((notification, index) => {
            const originalIndex = visibleNotifications.length - 1 - index;

            return (
              <div key={originalIndex} className={`p-4 border-b border-gray-200 flex items-center transition-transform duration-300 ${notification.removing ? 'transform -translate-x-full opacity-0' : 'opacity-100'}`}>
                <div>
                  <Image src="/svg/noPic.svg" className="rounded-full" height={45} width={45} alt="" />
                </div>
                <div className="ml-3 flex-1 text-sm max-w-full h-full">
                  <Link href={notification.redirect} onClick={closeNotifications}>
                    <span
                      className="block"
                      dangerouslySetInnerHTML={{
                        __html: `${notification.sender}: ${notification.message}`
                      }}
                    />
                  </Link>
                </div>
                {
                  notification.notificationType === 'connection-request' &&
                  <div className='flex items-center space-x-4'>
                    <a href={`/accept-connection/`}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7.29417 12.9577L10.5048 16.1681L17.6729 9" stroke="green" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                        <circle cx="12" cy="12" r="10" stroke="green" stroke-width="2" />
                      </svg>
                    </a>
                    <a href={`/decline-connection/`}>
                      <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <g id="Complete">
                          <g id="remove-circle">
                            <g>
                              <circle id="_--Circle" data-name="--Circle" cx="12" cy="12" r="10" fill="none" stroke="#000000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                              <line x1="15.5" y1="12" x2="8.5" y2="12" fill="none" stroke="#000000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                            </g>
                          </g>
                        </g>
                      </svg>
                    </a>
                  </div>
                }
                <svg className='ml-4 cursor-pointer' onClick={() => handleRemoveNotification(originalIndex)} width="24" height="24" viewBox="0 0 28 28" fill="red" xmlns="http://www.w3.org/2000/svg" >
                  <path d="M11.8489 22.6922C11.5862 22.7201 11.3509 22.5283 11.3232 22.2638L10.4668 14.0733C10.4392 13.8089 10.6297 13.5719 10.8924 13.5441L11.368 13.4937C11.6307 13.4659 11.8661 13.6577 11.8937 13.9221L12.7501 22.1126C12.7778 22.3771 12.5873 22.614 12.3246 22.6418L11.8489 22.6922Z" fill="red" />
                  <path d="M16.1533 22.6418C15.8906 22.614 15.7001 22.3771 15.7277 22.1126L16.5841 13.9221C16.6118 13.6577 16.8471 13.4659 17.1098 13.4937L17.5854 13.5441C17.8481 13.5719 18.0387 13.8089 18.011 14.0733L17.1546 22.2638C17.127 22.5283 16.8916 22.7201 16.6289 22.6922L16.1533 22.6418Z" fill="red" />
                  <path clipRule="evenodd" d="M11.9233 1C11.3494 1 10.8306 1.34435 10.6045 1.87545L9.54244 4.37037H4.91304C3.8565 4.37037 3 5.23264 3 6.2963V8.7037C3 9.68523 3.72934 10.4953 4.67218 10.6145L7.62934 26.2259C7.71876 26.676 8.11133 27 8.56729 27H20.3507C20.8242 27 21.2264 26.6513 21.2966 26.1799L23.4467 10.5956C24.3313 10.4262 25 9.64356 25 8.7037V6.2963C25 5.23264 24.1435 4.37037 23.087 4.37037H18.4561L17.394 1.87545C17.1679 1.34435 16.6492 1 16.0752 1H11.9233ZM16.3747 4.37037L16.0083 3.50956C15.8576 3.15549 15.5117 2.92593 15.1291 2.92593H12.8694C12.4868 2.92593 12.141 3.15549 11.9902 3.50956L11.6238 4.37037H16.3747ZM21.4694 11.0516C21.5028 10.8108 21.3154 10.5961 21.0723 10.5967L7.1143 10.6285C6.86411 10.6291 6.67585 10.8566 6.72212 11.1025L9.19806 24.259C9.28701 24.7317 9.69985 25.0741 10.1808 25.0741H18.6559C19.1552 25.0741 19.578 24.7058 19.6465 24.2113L21.4694 11.0516ZM22.1304 8.7037C22.6587 8.7037 23.087 8.27257 23.087 7.74074V7.25926C23.087 6.72743 22.6587 6.2963 22.1304 6.2963H5.86957C5.34129 6.2963 4.91304 6.72743 4.91304 7.25926V7.74074C4.91304 8.27257 5.34129 8.7037 5.86956 8.7037H22.1304Z" fill="red" fillRule="evenodd" />
                </svg>
              </div>
            );
          })
        )}
      </div>
    </div >
  );
};

export default NotificationComponent;
